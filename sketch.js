...
function drawGalerie(){
  ...
  // ===== zone dispo =====
  let usableW = width;
  let usableH = height - topArea - bottomArea;


  // taille max d’un carré
  let sizeFromW = (usableW - gap*(cols+1)) / cols;
  let sizeFromH = (usableH - gap*(rows+1)) / rows;

  let cellSize = 85;
  

  // ===== calcul centré PARFAIT =====
  ...
  let startX = cx - gridW/2+35; // centrage horizontal de la grille ( modifiable avec le +...)
  let startY = cy - gridH/2+20; // centrage verticale de la grille ( modifiable par le +...)
  ...
  for(let i=0;i<9;i++){
    ...
    // image
    image(galleryImages[i], x, y, cellSize, cellSize);
