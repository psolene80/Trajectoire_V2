...
function drawGalerie() {
  ...
  // taille des cases (tu avais fixé 85)
  let cellSize = 85;

  // ===== calcul centré PARFAIT (garde ton décalage d'origine) =====
  let gridW = cols * cellSize + gap * (cols - 1);
  let gridH = rows * cellSize + gap * (rows - 1);

  let cx = width / 2;
  let cy = height / 2;

  let startX = cx - gridW / 2 + 35; // centrage horizontal + décalage d'origine
  let startY = cy - gridH / 2 + 20; // centrage vertical + décalage d'origine
  ...
  for (let i = 0; i < 9; i++) {
    ...
    let scaleFactor = min(cellSize / img.width, cellSize / img.height);

    let imgW = img.width * scaleFactor;
    let imgH = img.height * scaleFactor;

    // centrer l'image dans le cadre
    let imgX = x + (cellSize - imgW) / 2;
    let imgY = y + (cellSize - imgH) / 2;

    image(img, imgX, imgY, imgW, imgH);

    // cadre (reste exactement comme avant)
    noFill();
    stroke(200);
    rect(x, y, cellSize, cellSize);

    // légende directement dessous
    noStroke();
    fill(0);
    text(galleryCaptions[i], x + cellSize / 2 - 43, y + cellSize + 4 - 40);
  }
